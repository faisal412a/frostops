/**
 * FrostOps ERP — Backend Server v2.1
 * Zero npm dependencies · Node.js built-ins only
 * Added: session auth, login/logout, role-based access, db export endpoint
 *
...
// Railway & Render have read-only app dirs — use /tmp (always writable).
// Locally stays next to server.js.
...
function getSession(req) {
  ...
  const match  = cookie.match(/frostops_session=([a-f0-9]{64})/);
  if (!match) return null;
  const session = SESSIONS.get(match[1]);
  if (!session) return null;
  if (Date.now() - session.createdAt > SESSION_TTL) { SESSIONS.delete(match[1]); return null; }
...
function seedDB() {
  const db = {
    ...
    salesOrders: [
      { id:'SO-1024', date:'2025-04-01', customerId:'CUST-001', customerName:'FreshMart UAE',   items:[{productName:'Chicken Nuggets 1kg',qty:50,unitPrice:47,total:2350},{productName:'Fish Fingers 500g',qty:50,unitPrice:33.5,total:1675}], subtotal:4025, tax:603.75, total:4628.75, payment:'Bank Transfer', status:'Shipped',    deliveryDate:'2025-04-05', notes:'' },
      { id:'SO-1025', date:'2025-04-02', customerId:'CUST-002', customerName:'CoolStore KSA',   items:[{productName:'Beef Burgers 4pk',qty:20,unitPrice:52.5,total:1050},{productName:'Frozen Peas 500g',qty:100,unitPrice:13,total:1300}], subtotal:2350, tax:352.50, total:2702.50, payment:'Credit 30 Days', status:'Processing', deliveryDate:'2025-04-08', notes:'' },
      { id:'SO-1026', date:'2025-04-03', customerId:'CUST-003', customerName:'Arctic Foods Ltd',items:[{productName:'Shrimp Cocktail 300g',qty:80,unitPrice:67.5,total:5400},{productName:'Cheese Pizza 400g',qty:50,unitPrice:37,total:1850}], subtotal:7250, tax:1087.50, total:8337.50, payment:'Net 30', status:'Pending',    deliveryDate:'2025-04-10', notes:'Cold chain required' },
      { id:'SO-1027', date:'2025-04-04', customerId:'CUST-004', customerName:'IceMart Qatar',   items:[{productName:'Chicken Nuggets 1kg',qty:80,unitPrice:47,total:3760}], subtotal:3760, tax:564, total:4324, payment:'Bank Transfer', status:'Processing', deliveryDate:'2025-04-09', notes:'' },
      { id:'SO-1028', date:'2025-03-28', customerId:'CUST-001', customerName:'FreshMart UAE',   items:[{productName:'Frozen Peas 500g',qty:200,unitPrice:13,total:2600}], subtotal:2600, tax:390, total:2990, payment:'Cash', status:'Delivered', deliveryDate:'2025-04-02', notes:'' },
    ],
    purchases: [
      { id:'PO-2025-001', date:'2025-04-01', supplierId:'SUP-001', supplierName:'Gulf Poultry Co.',   items:[{name:'Chicken Breast raw',qty:500,unit:'kg',unitPrice:18,total:9000}], subtotal:9000, tax:1350, total:10350, expectedDate:'2025-04-08', terms:'Net 30', status:'Received', notes:'' },
      { id:'PO-2025-002', date:'2025-04-02', supplierId:'SUP-002', supplierName:'Red Sea Seafood',    items:[{name:'White Fish Fillet',  qty:200,unit:'kg',unitPrice:24,total:4800}], subtotal:4800, tax:720,  total:5520,  expectedDate:'2025-04-07', terms:'Net 30', status:'Approved', notes:'' },
      { id:'PO-2025-003', date:'2025-03-25', supplierId:'SUP-003', supplierName:'Saudi Beef Trading', items:[{name:'Beef Mince 80/20',   qty:300,unit:'kg',unitPrice:32,total:9600}], subtotal:9600, tax:1440, total:11040, expectedDate:'2025-04-10', terms:'Net 60', status:'Pending',  notes:'' },
    ],
    invoices: [
...
function crud(collection, prefix, validators={}) {
  route('GET', `/api/${collection}`, (req, res) => {
    ...
    json(res, loadDB()[collection] || []);
  });
  route('GET', `/api/${collection}/:id`, (req, res, id) => {
    if (!requireAuth(req, res)) return;
    const item = (loadDB()[collection]||[]).find(i => i.id === id);
  ...
  route('POST', `/api/${collection}`, async (req, res) => {
    ...
    if (validators.create) { const e = validators.create(data, db); if (e) return err(res, e); }
    const item = { id: uid(prefix), ...data, createdAt: new Date().toISOString(), createdBy: session.username };
  ...
  route('PUT', `/api/${collection}/:id`, async (req, res, id) => {
    ...
    const data = await body(req);
    db[collection][idx] = { ...db[collection][idx], ...data, updatedAt: new Date().toISOString(), updatedBy: session.username };
  ...
  route('DELETE', `/api/${collection}/:id`, (req, res, id) => {
    ...
    if (session.role !== 'admin') return err(res, 'Admin access required', 403);
    const db = loadDB();
...
route('POST', '/api/auth/login', async (req, res) => {
  ...
  if (!user) {
    return err(res, 'Incorrect username or password', 401);
  }
  const token = createSession(user);
  res.writeHead(200, {
    'Content-Type': 'application/json',
    'Set-Cookie': `frostops_session=${token}; HttpOnly; Path=/; Max-Age=${12*3600}; SameSite=Lax`,
  });
  res.end(JSON.stringify({ ok: true, username: user.username, role: user.role, displayName: user.displayName }));
});
...
route('POST', '/api/auth/logout', (req, res) => {
  ...
  res.writeHead(200, {
    'Content-Type': 'application/json',
    'Set-Cookie': 'frostops_session=; HttpOnly; Path=/; Max-Age=0',
  });
...
route('GET', '/api/auth/me', (req, res) => {
  const session = getSession(req);
  if (!session) return err(res, 'Not authenticated', 401);
  json(res, { username: session.username, role: session.role, displayName: session.displayName });
});

// ─── DATA EXPORT ROUTE ──────────────────────────────────────
// GET /api/export — download full db.json as a file
route('GET', '/api/export', (req, res) => {
  if (!requireAuth(req, res)) return;
  const db = loadDB();
  const dateStr = new Date().toISOString().split('T')[0];
  const filename = `frostops-backup-${dateStr}.json`;
...
crud('salesOrders', 'SO', {
  afterCreate: (order, db) => {
    ...
    db.invoices.push({
      id: uid('INV-A'), date: order.date || today(),
...
// Purchases — auto-create supplier bill
crud('purchases', 'PO', {
  afterCreate: (po, db) => {
    db.supplierBills = db.supplierBills || [];
    db.supplierBills.push({
      id: uid('BILL'), date: po.date || today(),
      dueDate: addDays(po.date || today(), 30),
      supplierId: po.supplierId, supplierName: po.supplierName,
      purchaseId: po.id, total: po.total||0, paid: 0, status: 'Unpaid',
      createdAt: new Date().toISOString(),
    });
  }
});
...
route('GET', '/api/dashboard', (req, res) => {
  ...
  const revenue      = db.invoices.filter(i=>i.status==='Paid').reduce((s,i)=>s+(i.total||0),0);
  ...
  const now = new Date();
  const revenueChart = Array.from({length:8},(_,i)=>{
    const d = new Date(now.getFullYear(), now.getMonth()-(7-i), 1);
    const label = d.toLocaleString('en',{month:'short'});
    const value = db.invoices.filter(inv=>{
      if(inv.status!=='Paid') return false;
      const dt=new Date(inv.date); return dt.getMonth()===d.getMonth()&&dt.getFullYear()===d.getFullYear();
    }).reduce((s,inv)=>s+inv.total,0);
    return { label, value: Math.round(value) };
  });
  json(res, {
    invValue, receivables, payables, revenue, expenses, netProfit: revenue-expenses,
    lowStockCount: lowStock.length, expiringCount: expiring.length,
    overdueARCount: overdueAR.length, overdueBillsCount: overdueBills.length,
    totalCustomers: db.customers.length, totalSuppliers: db.suppliers.length,
    totalProducts: db.products.length, pendingOrders: db.salesOrders.filter(o=>o.status==='Pending').length,
    revenueChart, recentOrders: db.salesOrders.slice(-5).reverse(),
...
route('GET', '/api/receivables', (req, res) => {
  if (!requireAuth(req, res)) return;
  const db = loadDB();
  json(res, db.customers.map(c => {
    const unpaid = db.invoices.filter(i=>i.customerId===c.id&&i.status!=='Paid');
    return { ...c, totalReceivable: unpaid.reduce((s,i)=>s+(i.total-i.paid),0),
      overdueAmount: unpaid.filter(i=>daysUntil(i.dueDate)<0).reduce((s,i)=>s+(i.total-i.paid),0),
...
route('GET', '/api/payables', (req, res) => {
  if (!requireAuth(req, res)) return;
  const db = loadDB();
  json(res, db.suppliers.map(s => {
    const unpaid = db.supplierBills.filter(b=>b.supplierId===s.id&&b.status!=='Paid');
    return { ...s, totalPayable: unpaid.reduce((s,b)=>s+(b.total-b.paid),0),
      overdueAmount: unpaid.filter(b=>daysUntil(b.dueDate)<0).reduce((s,b)=>s+(b.total-b.paid),0),
...
const server = http.createServer((req, res) => {
  ...
  // API routes — no auth check here, each route does its own
  ...
  // login.html — embedded, never fails
  ...
  // Serve index.html for all app routes — embedded, never fails
  res.writeHead(200, { 'Content-Type':'text/html' });
  res.end(INDEX_HTML);
});
...
server.listen(PORT, () => {
  ...
  console.log(`   Mode:  Fully embedded — no public/ folder needed`);
